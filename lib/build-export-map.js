	...
	pagesData.map(page => {
		if (page.layout) {
			if (page.category !== "settings") {
				...
				if (page.bannerImage)
					pageMap.query.bannerImage = page.bannerImage;
				...
				if (page.ourWorkTitle)
					pageMap.query.ourWorkTitle = page.ourWorkTitle;
				if (page.position) pageMap.query.position = page.position;
				if (page.profileImage)
					pageMap.query.profileImage = page.profileImage;
				if (page.strap) pageMap.query.strap = page.strap;

				// Relational objects
				if (page.jobsRels)
					pageMap.query.jobsRels = buildRels({
						category: "jobs",
						rels: page.jobsRels
					});
				if (page.ourPeopleRels)
					pageMap.query.ourPeopleRels = buildRels({
						category: "our-people",
						rels: page.ourPeopleRels
					});
				if (page.ourWorkCategoryRels)
					pageMap.query.ourWorkCategoryRels = buildRels({
						category: "our-work",
						rels: page.ourWorkCategoryRels
					});
				if (page.ourWorkRels)
					pageMap.query.ourWorkRels = buildRels({
						category: "our-work",
						rels: page.ourWorkRels
					});
				if (page.testimonialsRels)
					pageMap.query.testimonialsRels = buildRels({
						category: "testimonials",
						rels: page.testimonialsRels
					});
...
function renameMediaFilesWithHash(exportMap) {
	...
	const markdownMediaRegex = new RegExp(
		`\\(${mediaPathRegex}(.*?)\\.*\\)`,
		"g"
	);
